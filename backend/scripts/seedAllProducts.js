/**
 * Seed All Products Script
 * Populates MongoDB with complete Arshi GPS product catalog & descriptions.
 * Run: node scripts/seedAllProducts.js
 */
  ...
  try {
    ...
    for (const catData of categoriesData) {
      ...
      if (!category) {
        ...
        console.log(`[SEED] Updated Category: ${category.name}`);
      }
    ...
    for (const prodData of productsData) {
      ...
      if (existing) {
        ...
        console.log(`[SEED] ✅ Updated Product: ${prodData.title}`);
      } else {
        await Product.create(productPayload);
        console.log(`[SEED] ✅ Created Product: ${prodData.title}`);
      }
    }

    console.log('[SEED] 🎉 All categories and products seeded successfully!');
  } catch (error) {
